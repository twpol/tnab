...
public class BoxParser
{
    ...
    List<CssStyleRule> StyleSheetRules = [];

    public BoxParser(Document document)
    {
        Document = document;
    ...
    public void Parse()
    {
        StyleSheetRules = [.. Document.OfType<TNABStyleSheet>().Select(node => node.StyleSheet).OfType<CssStyleSheet>().SelectMany(sheet => sheet.Rules).OfType<CssStyleRule>()];
    ...
    record BoxMeasure(Node Node, BoxStyle Style, SKSize MinContent, SKSize MaxContent, CustomList<BoxMeasure> Children);

    BoxMeasure Measure(CustomDictionary<string, CustomList<CssStyleValue>> parentStyles, Node node, SKRect rectangle)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(text.NodeValue))
            {
            ...
            var style = new BoxStyle("FIXME:", "FIXME:")
            {
                Color = color,
                Font = FontDefault,
                Text = new(new(text.NodeValue), null, null, null, null, null, null, null, null)
            };
    ...
    CustomDictionary<string, CustomList<CssStyleValue>> GetStyles(CustomDictionary<string, CustomList<CssStyleValue>> parentStyles, Node node)
    {
        ...
        foreach (var rule in StyleSheetRules)
        {
            if (rule.IsMatch(node))
            {
                foreach (var property in rule.Style.Properties)
                {
                    ...
                    styles[property.Name] = property.Values;
                }
            }
        }
        return styles;
